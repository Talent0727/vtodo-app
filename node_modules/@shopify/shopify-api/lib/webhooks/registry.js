            ...
            for (const [topic, handlers] of Object.entries(handlersToAdd)) {
                ...
                if (Array.isArray(handlers)) {
                    for (const handler of handlers) {
                        yield mergeOrAddHandler(config, webhookRegistry, topicKey, handler);
                    }
                }
                else {
                    yield mergeOrAddHandler(config, webhookRegistry, topicKey, handlers);
                }
            }
        });
    };
...
function mergeOrAddHandler(config, webhookRegistry, topic, handler) {
    ...
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        for (const index in webhookRegistry[topic]) {
            ...
            if (handler.deliveryMethod === types_1.DeliveryMethod.Http) {
                yield (0, logger_1.logger)(config).info(`Detected multiple handlers for '${topic}', webhooks.process will call them sequentially`);
                break;
            }
            else {
